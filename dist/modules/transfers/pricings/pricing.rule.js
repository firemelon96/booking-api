...
function validateTransferPricing(mode, pricing) {
    if (!pricing.length) {
    ...
    const grouped = (0, lodash_1.groupBy)(pricing, 'pricingType');
    for (const [type, ranges] of Object.entries(grouped)) {
        const sorted = [...ranges].sort((a, b) => a.minPassengers - b.minPassengers);
        ...
        for (let i = 0; i < sorted.length; i++) {
            ...
            if (current.minPassengers > current.maxPassengers) {
                throw new Error(`${type}: Invalid range ${current.minPassengers}=${current.maxPassengers}`);
            }
            if (current.price <= 0) {
            ...
            if (current.minPassengers <= prev.maxPassengers) {
                throw new Error(`${type}: Overlap between ${prev.minPassengers}-${prev.maxPassengers} and ${current.minPassengers}-${current.maxPassengers}`);
            }
            if (current.minPassengers !== prev.maxPassengers + 1) {
                throw new Error(`${type}: Gap between ${prev.maxPassengers} and ${current.minPassengers}`);
            }
        }
    }
}
