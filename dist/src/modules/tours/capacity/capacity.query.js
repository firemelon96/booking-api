...
exports.findCapacityOrFail = findCapacityOrFail;
exports.prepareCapacity = prepareCapacity;
exports.reserveCapacity = reserveCapacity;
exports.releaseCapacity = releaseCapacity;
exports.lockCapacityRows = lockCapacityRows;
const prisma_1 = require("../../../config/prisma");
const admin_warning_service_1 = require("../../logs/admin-warning.service");
const helper_1 = require("../../../utils/helper");
async function findCapacityOrFail({ id }) {
    const capacity = await prisma_1.prisma.tourDailyCapacity.findUnique({
        where: { id },
    });
    if (!capacity) {
...
async function prepareCapacity({ tx, tourId, scheduleId, capacity, dates, }) {
    const scheduleKey = (0, helper_1.getScheduleKey)(scheduleId);
...
async function reserveCapacity({ tx, rows, capacityMode, participants, pricingType, isAdmin, userId, }) {
    switch (capacityMode) {
        case 'EXCLUSIVE':
...
async function reserveMixSharedTourCapacity(tx, { rows, isAdmin, participants, userId, }) {
    let hasOverbooking = false;
    let hasAdminOverride = false;
...
async function reserveSharedTourCapacity(tx, { rows, isAdmin, participants, userId, }) {
    let hasOverbooking = false;
    let hasAdminOverride = false;
    for (const row of rows) {
...
async function reserveExclusiveTourCapacity(tx, { rows, isAdmin, userId, }) {
    let hasOverbooking = false;
    let hasAdminOverride = false;
    for (const row of rows) {
        const hasBooking = row.bookedSlots > 0;
...
// }
async function releaseCapacity({ tx, dates, participants, scheduleId, tourId, }) {
    for (const date of dates) {
...
async function lockCapacityRows(tx, { tourId, dates, scheduleId, }) {
    const scheduleKey = (0, helper_1.getScheduleKey)(scheduleId);
